...
# Allow imports from the project root (for config.settings)
...
# Faker instance with Indian locale for more realistic city/name data
fake = Faker("en_IN")
Faker.seed(42)        # Reproducible fake data
random.seed(42)

# ---------------------------------------------------------------------------
# Product data — realistic e-commerce catalogue
# ---------------------------------------------------------------------------
...
# Indian cities for realistic user data
INDIAN_CITIES = [
...
def generate_products(n: int) -> list[tuple]:
    ...
    for i in range(n):
        ...
        # Add a numeric suffix to differentiate repeated products
        suffix = f" v{i // len(catalogue_flat) + 1}" if i >= len(catalogue_flat) else ""
        name = name_template + suffix
        # Random price within the realistic range
        price = round(random.uniform(price_min, price_max), 2)
        # Stock: 0 (out of stock) or 5–500 units
...
def generate_orders(n: int, user_ids: list[int], product_ids: list[int]) -> list[tuple]:
    ...
    for _ in range(n):
        ...
        # Orders placed within the last 12 months (for date-based queries)
...
def generate_payments(
    ...
    for i, order_id in enumerate(order_ids):
        ...
        # Payment date: 0–3 days after the order
        if status == "paid":
            payment_date = order_dates[i] + timedelta(
                hours=random.randint(0, 72)
            )
        else:
            # Pending/failed: no payment date yet
            payment_date = None
...
def generate_reviews(n: int, product_ids: list[int], user_ids: list[int]) -> list[tuple]:
    ...
    # Rating distribution: skewed toward 4-5 stars (realistic)
    rating_pool = [1] * 5 + [2] * 10 + [3] * 15 + [4] * 30 + [5] * 40

    # Short review templates
    positive_comments = [
...
# ---------------------------------------------------------------------------
# Existence checks (skip if data already present)
# ---------------------------------------------------------------------------
...
def main(reset: bool = False):
    ...
    # Step 1: Create database (connect without selecting a DB first)
    ...
    # Step 3: Optionally reset (drop all tables)
    if reset:
    ...
    # Step 5: Insert data (skip tables that already have data unless --reset)
    ...
    # Build a price map: product_id → price (needed to calculate payment amounts)
    ...
    # Fetch orders to build payment data (need order_id, product_id, quantity, order_date)
